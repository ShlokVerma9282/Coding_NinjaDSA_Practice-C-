...
void reverse(BinaryTreeNode<int> *root)
{
    if (root == NULL)
    {
        return;
    ...
    while (pending_Node.size() != 0)
    {
        ...
        cout << head->data << ":";
        pending_Node.pop();
        BinaryTreeNode<int> *leftChild = head->left;
        BinaryTreeNode<int> *rightChild = head->right;
        if (head->left != NULL)
        {
            head->right = rightChild;
            cout << head->right->data<<",";
            pending_Node.push(rightChild);
        }
        if (head->left == NULL)
        {
            cout << -1 << ",";
