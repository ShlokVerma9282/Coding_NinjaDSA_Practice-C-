...
void print_in_range_BST(BinaryTreeNode<int>*root,int x,int y){
    if ( root==NULL ){  
        return;  
    }  
  
    if ( x < root->data ){  
        print_in_range_BST(root->left, x, y);  
    }
    ...
    if ( y > root->data ){  
        print_in_range_BST(root->right, x, y);  
    }
